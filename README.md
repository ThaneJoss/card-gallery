...
3. 保存后运行 `pnpm check:cards` 检查资料并提交 PR；bot 会把远程卡面存入仓库，再更新「图片」字段。确认 bot 的提交后合并 PR，重新部署后线上会更新。本地预览可先运行 `pnpm localize:cards`，再按下方构建与预览说明启动网站。
...
| 贴图四角 | 否 | 实物照片中的卡面区域，按左上、右上、右下、左下填写四个 `[x, y]`；坐标除以图片宽高，范围为 0–1。只影响三维纹理采样，省略时使用整张图片。更换照片后需重新标记或删除。 |
...
cards.yaml 中的中文卡片资料和本地图片路径
  ...
  → 打包 Three.js 与三维查看器为 public/card-viewer.js，首次打开详情时加载
  → 发布整个 public/
```
...
- `pnpm test:browser` 构建后运行真实 Chromium 回归，检查模型与画布复用、换卡不重传几何、照片贴图、正反面材质、完整旋转、复位、旧纹理请求竞争、手机触摸和焦点恢复。首次使用需运行 `pnpm exec playwright install chromium` 安装测试浏览器；截图保存在已忽略的 `output/card-viewer/`。
...
- 点击卡片进入三维详情，拖动鼠标或手指让相机上下、左右完整环绕；模型和光源在环绕过程中固定。所有银行卡复用一个模型，尺寸、圆角、厚度不随图片比例改变，切卡只更新正面纹理及横竖摆放方向。
- 哑光、覆膜、虹彩三种材质同时应用到正反面，背面为无文字的中性色；这些效果用于展示，不表示已采集真实卡片工艺。
- “查看另一面”环绕到相反视角，“复位视角”返回初始位置。聚焦卡面后可用方向键旋转、Home 复位；减少动态效果偏好会关闭翻面的过渡动画。
- 三维资源按需加载；关闭详情时释放卡面纹理并停止动画，保留唯一的模型、画布与渲染器供下次使用，页面退出时统一释放。快速换卡会丢弃旧的加载结果；设备不支持三维显示或资源加载失败时仍可查看原图。
