...
- **图片格式与比例**：可使用浏览器支持的 WebP、PNG、JPG 等图片。列表使用 `4 : 3` 浅灰展台，卡面和大图均按原始比例完整展示，不裁切、不拉伸；原图自带留白也会保留。
...
- `pnpm test:browser` 构建后运行真实 Chromium 回归，检查正反面材质的实际画面差异、完整旋转、复位、换卡、手机触摸和焦点恢复。首次使用需运行 `pnpm exec playwright install chromium` 安装测试浏览器；截图保存在已忽略的 `output/card-viewer/`。
...
| `src/card-viewer.js` | 三维卡片几何、相机环绕、正反面材质与资源释放 |
