...
- **调整顺序**：银行按 `cards.yaml` 中首次出现的顺序分组。桌面四列固定为 UnionPay（银联）、Visa、Mastercard、AMEX，缺失组织保留空位；储蓄卡在前，信用卡另起一排。同一类型、同一卡组织的多张卡按文件顺序向下排列，双标卡归入上述优先级最高的组织。JCB、Discover 和未填写组织的卡片在「其他卡组织」区域展示。筛选后仍保持固定列位。
