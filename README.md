...
- **图片格式与比例**：可使用浏览器支持的 WebP、PNG、JPG 等图片。列表使用 `4 : 3` 浅灰展台，完整保留原图。三维详情把卡面作为纹理映射到统一比例的卡片模型上；横竖设计共用同一几何，竖版仅调整摆放朝向与纹理坐标。
- **实物照片贴图**：带桌面或透视倾斜的照片可填写「贴图四角」，查看器将指定四边形透视映射为卡面纹理。锦鲤卡已配置；照片原件和列表展示保持不变。该处理保留拍摄时的光照，不会还原被遮挡的图案。
...
- 冷灰页面 `#f6f7f9`、白色展台和面板 `#ffffff`、深蓝灰文字 `#182230`、辅助文字 `#526075`、蓝色强调 `#2563eb`；选中状态采用浅蓝 `#eff4ff` 配深蓝文字 `#1d4ed8`，边框为 `#e2e6ec`。颜色、字体、间距与动效变量集中在 `styles.css` 的 `:root` 中。
- 内容最大宽度为 1160px，标题、工具栏、结果说明、画廊和页脚共用对齐线。桌面三列、600–1023px 两列、小于 600px 单列。
- 收藏条目采用开放式排版：完整卡面下方展示卡名、银行、卡种和已有卡组织，长文字自然换行。白色展台带细边框与轻阴影；控件、展台、弹层圆角分别为 8px、16px、16px，列表卡面为 10px，大图卡面为 16px。详情面板内使用浅灰 `#f1f3f6` 承托卡面。
...
- 画廊加载后在空闲时段准备共用三维场景，打开详情时直接展示卡面，三维就绪后切换，不显示等待提示。关闭详情只隐藏模型并停止动画，保留模型、画布及最近使用的贴图；贴图缓存按约 96 MiB 的 RGBA 与 mipmap 估算预算淘汰最久未用项，当前贴图始终保留，页面退出时统一释放。快速换卡会丢弃过期结果；设备不支持三维显示或资源加载失败时仍可查看原图。
