...
- **调整顺序**：银行按 `cards.yaml` 中首次出现的顺序分组，每家银行独立起行。组内按 UnionPay（银联）、Visa、Mastercard、AMEX 排列；JCB、Discover 和未填写卡组织的卡片依次排在其后。同一卡组织保留文件顺序，双标卡按上述优先级最高的卡组织排序。筛选后仍保留此顺序。
...
- 内容最大宽度为 1480px，标题、工具栏、结果说明、画廊和页脚共用对齐线。1920×1080 浏览器默认四列；1280px 及以上四列、1024–1279px 三列、600–1023px 两列、小于 600px 单列。每家银行独立起行，不足四张保留空位，超过四张在本银行组内续行。
