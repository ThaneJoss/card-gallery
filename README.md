...
- 依据《「卡间拾光」视觉设计改版说明》采用“私人收藏馆 × 编辑式画廊”风格。首页以一个宋体风格的站名为主标题，点击可返回本站 `./index.html`；保留“叠卡拾光”矢量 favicon。
- 使用本地字体回退，正文和控件保持黑体风格，不加载在线字体或新的 UI 库。
- 暖白页面 `#f7f6f2`、浅灰展台 `#eeede8`、炭黑文字 `#242421`、暗朱红状态色 `#934b40`；颜色、字体、间距与动效变量集中在 `styles.css` 的 `:root` 中。
- 内容最大宽度为 1160px，标题、工具栏、结果说明、画廊和页脚共用对齐线。桌面三列、600–1023px 两列、小于 600px 单列。
- 收藏条目采用开放式排版：完整卡面下方展示卡名、银行、卡种和已有卡组织，长文字自然换行。控件、展台、弹层圆角分别为 6px、8px、12px；按补充要求保留银行卡式圆角，列表卡面为 10px，大图卡面为 16px。
- 仅在精细鼠标悬停时让卡面上移 3px，文字和展台不移动；减少动态效果偏好会关闭位移、过渡与淡入。
- 本轮实际验收记录见 [视觉改版验收](docs/visual-redesign-checks.md)。
